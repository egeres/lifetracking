...
def get_latest_git_tag():
    result = subprocess.run(
        ["git", "describe", "--abbrev=0", "--tags"], capture_output=True, text=True
    )
...
def update_version_in_pyproject_toml(version):
    ...
    if version.startswith("v"):
        version = version[1:]
    content["tool"]["poetry"]["version"] = version
...
def main():
    ...
    if not re.match(r"^v?\d+\.\d+\.\d+$", latest_tag):
        raise Exception(
            f"Invalid Git tag: {latest_tag}. Must be in semantic versioning format (e.g., 1.2.3)"
        )
