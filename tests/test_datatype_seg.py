...
def test_basic_creation():
    ...
    a = Seg(
        ...
        datetime(2021, 1, 1, 23, 59, 59, 999999),
    )
...
def test_seg_add():
    ...
    assert b.start == datetime(2021, 1, 2)
    assert b.end == datetime(2021, 1, 3)
...
def test_seg_sub():
    ...
    # Remove 1 day
    b = a - timedelta(days=1)
    assert b.start == datetime(2020, 12, 31)
    assert b.end == datetime(2021, 1, 1)
...
def test_seg_split():
    ...
    assert len(b) == 3
    # fmt: off
    assert b[0].start == datetime(2021, 1, 1, 12)
    assert b[0].end   == datetime(2021, 1, 1, 23, 59, 59, 999999)
    assert b[0].value == {"1+1": "2"}
    assert b[1].start == datetime(2021, 1, 2)
    assert b[1].end   == datetime(2021, 1, 2, 23, 59, 59, 999999)
    assert b[1].value == {"1+1": "2"}
    assert b[2].start == datetime(2021, 1, 3)
    assert b[2].end   == datetime(2021, 1, 3, 12)
...
def test_seg_overlap():
    #           |---a---|
    # |---b---|
    a = Seg(datetime(2010, 1, 1), datetime(2010, 6, 1))
    b = Seg(datetime(2000, 1, 1), datetime(2000, 6, 1))
    assert not a.overlaps(b)
    assert not b.overlaps(a)

    #           |---a---|
    #             |-c-|
    c = Seg(datetime(2010, 3, 1), datetime(2010, 4, 1))
    assert c.overlaps(a)
    assert a.overlaps(c)

    #           |---a---|
    #      |---d---|
    d = Seg(datetime(2009, 8, 1), datetime(2010, 3, 1))
    assert d.overlaps(a)
    assert a.overlaps(d)

    #           |---a---|
    #               |---e---|
    e = Seg(datetime(2010, 3, 1), datetime(2010, 9, 1))
    assert e.overlaps(a)
    assert a.overlaps(e)
